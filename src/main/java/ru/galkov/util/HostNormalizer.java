...
/**
 * Strict normalization helpers for hosts, IP literals and host:port values.
 *
...
public final class HostNormalizer {
    ...
    private static byte[] parseIpv4Literal(String value) {
        ...
        for (
            ...
            /*
             * Запрещаем неоднозначные IPv4-формы:
             *
             * 08.08.08.08
             * 001.2.3.4
             *
             * Разрешается только обычный dotted-decimal:
             *
             * 8.8.8.8
             */
            if (
    ...
    private static byte[] parseIpv6Literal(String value) {
        ...
        int leftGroups =
                countIpv6Groups(left);

        int rightGroups =
                countIpv6Groups(right);
    ...
    private static int writeIpv6Groups(
            String part,
            byte[] target,
            int position
    ) {
    ...
    private static boolean isValidDnsLabel(
            String label
    ) {
        if (
                label == null
