...
public final class BlacklistSnapshot {
    ...
    private static final int DEFAULT_MAX_IP_CACHE_SIZE = 100_000;
    private static final int DEFAULT_MAX_DOMAIN_CACHE_SIZE = 200_000;
    ...
    public BlacklistSnapshot(DomainTrie domainTrie, Set<String> ipSet, Set<IpCidr> cidrSet, Map<String, String> domainSources) {
        ...
        this.maxIpCacheSize =
                positiveInt(config, "blacklist.snapshot.max-ip-cache-size", DEFAULT_MAX_IP_CACHE_SIZE);

        this.maxDomainCacheSize =
                positiveInt(config, "blacklist.snapshot.max-domain-cache-size", DEFAULT_MAX_DOMAIN_CACHE_SIZE);
        ...
        this.ipCache = createLruCache(maxIpCacheSize);
        this.domainCache = createLruCache(maxDomainCacheSize);
    }

    public static BlacklistSnapshot empty() {
        return new BlacklistSnapshot(null, Collections.emptySet(), Collections.emptySet());
    }
    ...
    public BlockDecision checkIp(String ip) {
        ...
        CacheEntry cached = ipCache.get(normalized);

        if (cached != null) {
            if (!cached.expired(cacheTtlMillis)) return cached.decision;
            ipCache.remove(normalized, cached);
        }

        BlockDecision decision;
        if (exactIps.contains(normalized)) {
            decision = BlockDecision.blockIpExact(normalized, "blacklist");
        } else {
            try {
                InetAddress address = InetAddress.getByName(normalized);

                IpCidr matchedCidr = ipPrefixIndex.findMatch(address).orElse(null);

                decision =
                        matchedCidr == null ? BlockDecision.allow() :
                                BlockDecision.blockIpCidr(matchedCidr.toString(), "blacklist");

            } catch (Exception e) {
                logger.debug("Unable to check IP against CIDR index: {}", normalized, e);
                decision = BlockDecision.allow();
            }
        }
        putCache(ipCache, normalized, decision, maxIpCacheSize);
        return decision;
    }
    ...
    public BlockDecision checkDomain(String domain) {
        ...
        if (normalized == null) return BlockDecision.allow();
        CacheEntry cached = domainCache.get(normalized);
    ...
    private static Set<String> immutableIpSet(Set<String> values) {
        Set<String> result = ConcurrentHashMap.newKeySet();

        if (values != null) {
            for (String value : values) {
                String normalized = HostNormalizer.normalizeIp(value);
    ...
    private static Set<IpCidr> immutableCidrSet(Set<IpCidr> values) {
        Set<IpCidr> result = ConcurrentHashMap.newKeySet();
        if (values != null) result.addAll(values);
        return Collections.unmodifiableSet(result);
    }

    private static Map<String, CacheEntry> createLruCache(int maxSize) {
    ...
    private static void putCache(Map<String, CacheEntry> cache, String key, BlockDecision decision, int maxSize) {
        synchronized (cache) {
            cache.put(key, new CacheEntry(decision));
            if (cache.size() > maxSize) cache.remove(cache.keySet().iterator().next());
        }
    }
    ...
    private static int positiveInt(AppConfig config, String key, int fallback) {
        if (config == null) return fallback;
        try {
            return Math.max(1, config.getInt(key));
        } catch (RuntimeException e) {
            return fallback;
        }
    }

    private static long positiveLong(AppConfig config, String key, long fallback) {
        if (config == null) return fallback;

        try {
            return Math.max(1L, config.getLong(key));
        } catch (RuntimeException e) {
            return fallback;
        }
    }
    ...
    private static final class CacheEntry {
        ...
        private CacheEntry(BlockDecision decision) {
