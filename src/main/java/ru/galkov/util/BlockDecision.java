...
public final class BlockDecision {
    ...
    private static final BlockDecision ALLOW_DECISION =
            new BlockDecision(false, BlockReason.NONE, null, null, BlockAction.ALLOW);
    ...
    public static BlockDecision blockDomain(DomainTrie.MatchType type, String rule, String source) {
        ...
        return blocked(reason, rule, source);
    }

    private static BlockDecision blocked(BlockReason reason, String rule, String source) {
        return new BlockDecision(
                true,
                reason,
                rule,
                source == null ? "blacklist" : source, BlockAction.BLOCK_HTTP_403);
    }
    ...
    public String toString() {
        if (!blocked) return "BlockDecision{ALLOW}";
