...
┌─────────────────-------------┐
│  DNS Server и HHTP proxy     │
│  (существующий)              │
└────────┬────────-------------┘
│
▼ (асинхронно, копия запросов)
┌─────────────────────────────────────┐
│  DnsAnomalyDetector                 │
│  ┌───────────────────────────────┐  │
│  │  RequestBuffer (очередь)      │  │
│  └──────────────┬────────────────┘  │
│                 │                   │
│                 ▼                   │
│  ┌───────────────────────────────┐  │
...
│  ┌───────────────────────────────┐  │
│  │  DynamicBlocklist             │  │
│  │  - оперативный список блоки   │  │
│  │  - потокобезопасный           │  │
│  └───────────────────────────────┘  │
└─────────────────────────────────────┘
│
▼ (читают DNS и HTTP Proxy)
┌─────────────────┐     ┌──────────────┐
│  DNS Server     │     │  HttpProxy   │
│  (проверка)     │     │  (проверка)  │
└─────────────────┘     └──────────────┘
